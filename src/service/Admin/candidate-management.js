...
export const getAlCandidatesAdmin = (filters = {}) =>
...
export const getCandidateDetailAdmin = (candidate_id) =>
    new Promise(async (resolve) => {
        try {
            const candidateRepo = new CandidateRepository();
            const candidate =
                await candidateRepo.getDetailByCandidateId(candidate_id);
            if (!candidate)
...
export const deleteCandidateAdmin = (candidate_id) =>
    new Promise(async (resolve) => {
        ...
        try {
            ...
            try {
                await db.Interview.destroy({ where: { candidate_id }, transaction });
            } catch { }
            try {
            ...
            try {
                await db.SupportRequest.destroy({
...
export const updateCandidateAdmin = (candidate_id, payload) =>
    new Promise(async (resolve) => {
        console.log(
            "updateCandidateAdmin called with candidate_id:",
            candidate_id,
            "payload:",
            payload,
        );
        ...
        try {
            ...
            if (candidateInfo) {
                ...
                await db.Candidate.update(nextCandidateInfo, {
                    where: { candidate_id },
                    transaction,
                });
            } else if (email) {
            ...
            if (addressInfo) {
                ...
                if (candidate.address_id) {
                    await db.Address.update(normalizedAddressInfo, {
                        where: { id: candidate.address_id },
                        transaction,
                    });
                } else {
                    const created = await db.Address.create(normalizedAddressInfo, {
                        transaction,
                    });
                    await db.Candidate.update(
            ...
            if (Array.isArray(studyHistories)) {
                ...
                const rows = studyHistories.map((h) => ({
                    ...
                    start_date:
                        h.start_date ?? (h.start_year ? `${h.start_year}-01-01` : null),
                    end_date: h.end_date ?? (h.end_year ? `${h.end_year}-01-01` : null),
                }));
                if (rows.length > 0)
                    await db.StudyHistory.bulkCreate(rows, { transaction });
            }
            ...
            if (Array.isArray(workExperiences)) {
                ...
                if (rows.length > 0)
                    await db.WorkExperience.bulkCreate(rows, { transaction });
            }

            await transaction.commit();
            console.log(
                "updateCandidateAdmin success for candidate_id:",
                candidate_id,
            );
            return resolve({ err: 0, mes: "Cập nhật ứng viên thành công" });
        } catch (error) {
            console.log("updateCandidateAdmin error:", error);
