import { CandidateRepository, UserRepository } from "../../repository/index.js";
...
export class CandidateAuthBuilder {
    constructor() {
    ...
    async verifiedCallBackCandidate() {
        ...
        const user = await this.candidateRepo.getByEmail(email);
        if (!user)
    ...
    async loginCandidate() {
        ...
        const user = await this.candidateRepo.getByEmail(this.email);
        if (!user)
            throw new Error(`Không tìm thấy thông tin ứng viên`);
        if (!user.candidate)
            throw new Error("Không tìm thấy thông tin ứng viên");
    ...
    async forgotPasswordCandidate() {
        if (!this.email) throw new Error("Missing email");

        const user = await this.candidateRepo.getByEmail(this.email);
        if (!user)
            throw new Error("Không tìm thấy ứng viên với email này");
    ...
    async logout() {
        ...
        const user = await this.candidateRepo.getById(this.user_id);
        if (!user)
            throw new Error("Không tìm thấy ứng viên với email này");
